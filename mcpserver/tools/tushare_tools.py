...
def get_stock_basic(name: str = Field(description='公司名称')) -> str:
    ...
    # 拉取数据
    tushare_client = TuShareFactory.build_api_client()
    df = tushare_client.stock_basic(**{"name": name}, fields=[ "ts_code", "name", "industry", "market"])
    return df.to_json(orient='records')

@tools.append
def get_daily_basic(name: str = Field(description='公司名称'),
                    limit: int = Field(description='交易日数量，用于指定查询近几个交易日的数据', default=10)) -> str:
    """ 根据公司名称获取最近若干交易日的股票交易信息，包含收盘价、换手率、市盈率、市销率、流通股本、流通市值、总市值等等 """
    tushare_client = TuShareFactory.build_api_client()
    basic = tushare_client.stock_basic(**{"name": name}, fields=["ts_code"])
    ts_code = basic.iloc[0]['ts_code']
    fields = list(daily_basic_field_map.keys())
    df = tushare_client.daily_basic(**{"ts_code": ts_code, "limit": limit}, fields=fields)

    df_chinese = _replace_columns_with_chinese(df, daily_basic_field_map)
    return df_chinese.to_json(orient='records')

@tools.append
def get_fina_indicator(name: str = Field(description='公司名称'),
                       period: str = Field(description='上市公司发布的财务报表报告期，以yyyyMMdd格式传入。12月31日为年报，3月31日为一季报以此类推')) -> str:
    """ 根据公司名称获取公司财务指标信息 """
    tushare_client = TuShareFactory.build_api_client()
    basic = tushare_client.stock_basic(**{"name": name}, fields=["ts_code"])
    ts_code = basic.iloc[0]['ts_code']
    fields = list(daily_basic_field_map.keys())
    df = tushare_client.fina_indicator(**{"ts_code": ts_code, "period":period, "update_flag":1}, fields=fields)
...
def _replace_columns_with_chinese(df: DataFrame, field_map: dict) -> DataFrame:
    """
    将DataFrame的列名替换为field_map中的中文含义

    Parameters:
    df: pandas DataFrame, API返回的数据
    field_map: dict, 字段映射字典，格式为 {'英文字段名': '中文含义'}

    Returns:
    pandas DataFrame: 列名已替换为中文的DataFrame
    """
    # 创建新的列名映射
    new_columns = []
    for col in df.columns:
        if col in field_map:
            new_columns.append(field_map[col])
        else:
            new_columns.append(col)  # 如果field_map中没有对应字段，保留原列名

    # 复制DataFrame并替换列名
    df_chinese = df.copy()
    df_chinese.columns = new_columns
