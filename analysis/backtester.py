...
class Backtester:
    ...
    def run_multi_index(self, 
                       index_data_list: List[dict],
                       positions_dict: dict = None) -> dict:
        """
        多指数回测
        
        Args:
            index_data_list: 各指数的数据列表，每个元素包含:
                {
                    'df': DataFrame,
                    'signal_column': str,
                    'name': str,
            ...
            positions_dict: 每日仓位配置 (可选)
                [
                    {
                        'date': datetime,
                        'positions': {'000300.SH': 0.3, '399006.SZ': 0.2},
                        'cash': 0.5
                    },
                    ...
                ]
        ...
        total_value = total_initial_capital
        total_commission = 0.0
        
        # 为每个指数初始化回测器
        index_results = {}
        
        for idx_data in index_data_list:
            df = idx_data['df']
            signal_col = idx_data['signal_column']
            code = idx_data['code']
            
            # 运行单指数回测
            result = self.run(df, signal_column=signal_col)
            index_results[code] = result
            
            print(f"  {idx_data['name']} ({code}): {result['total_return']:>7.1f}%")
        
        # 如果没有提供 positions_dict，使用等权重 allocating
        if positions_dict is None:
            # 简单等权重：每个指数持仓 1 / len(index_data_list)
            # 等等，这不符合我们的仓位管理逻辑
            # 应该基于每个指数的收益率来分配
            pass
        ...
        return {
            ...
            'total_return': sum(r['total_return'] for r in index_results.values()) / len(index_results),
            'annualized_return': 0.0,  # 需要更复杂的计算
            'max_drawdown': 0.0,
            'sharpe_ratio': 0.0,
        }
