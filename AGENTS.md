...
## 28. V21 Regime状态特征增强 (2026-04-30)

### 优化内容
在ML预测中加入3个Regime状态特征，增强模型对市场环境的感知能力。

**新增特征**:
1. **regime_duration** (持续时长): 连续同状态天数，SIDEWAYS均值44.7d vs HIGH_VOL均值3.9d
   - 实现: `_calc_regime_duration()` 按状态分组累计计数
2. **regime_transition_stay/flip** (转换概率): 滚动60天窗口内，同状态次日停留/切换概率
   - 实现: `_calc_transition_prob_by_target()` 滚动窗口，min_samples=10
3. **feat_cross_50_regime_consistency** (跨指数一致性): 当前指数与上证50的regime类别一致性
   - 20=牛熊互斥, 50=部分一致, 70=同类不同级, 90=完全一致

### 涉及文件
- `analysis/market_regime_detector.py` — 新增2个方法, detect()新增3个新列
- `analysis/ml_predictor.py` — prepare_features()新增feat_regime_duration/stay_prob/flip_prob
- `analysis/index_analyzer.py` — _apply_cross_index_features()新增上证50regime一致性计算

### 验证测试
- 持续时长: 513天(最长)正确计数, HIGH_VOL均值3.9d ↔ SIDEWAYS均值44.7d (区分度良好)
- 转换概率: stay+flip≈100自洽, SIDEWAYS停留率98.8%, HIGH_VOL翻转率50.0%
- 一致性: 大/小盘65.4%完全一致, 仅0.6%牛熊互斥(合理)
- ML回测方向准确率:~52.8% (与基线~53.7%持平)

### 回测结果 (2020-01 ~ 2026-04)
- 总收益: **+74.71%**, 年化: **+9.64%**
- 最大回撤: **-18.64%**, 夏普: **0.57**
- 月度胜率: 53.9%, 超额收益: +31.25%
...
- 收益率基本持平(在ML随机性波动范围内)
- 回撤控制改善5.86pp ✅
- **方向准确率未显著提升** — 新特征与现有35个特征高度相关，特征筛选(2026-03-30)已验证仅靠添加/删除特征难以突破方向准确率天花板
