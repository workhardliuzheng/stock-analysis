...
> **核心瓶颈**: ML方向准确率长期卡在 **53-54%** 无法突破。以下方向均已验证无效，**不要重复尝试**。

| 方向 | 验证时间 | 结果 |
|------|---------|------|
| **添加/删除ML特征** | 2026-03-30 (特征筛选) | 35→20个特征，方向准确率不变(53.69%) |
| **ML信号融合规则替换** | 2026-04-30 (V20) | XGBoost替换if-else，收益下降18pp |
| **V21新加3个regime特征** | 2026-04-30 | 方向准确率~52.8%，持平 |
| **Optuna特征权重优化** | (未正式尝试，但特征筛选已等价验证) | 特征集合调整效果有限 |

### ✅ 真正有效的优化（按效果排序）

| 优化 | 收益提升 | 回撤改善 |
|------|---------|---------|
| **黄金ETF防御资产 (V19)** | +48.24pp | -9.46pp |
| V12三层下跌保护 | +2.22pp | -10.65pp |
| V14防鞭打增强 | +9.77pp | -1.04pp |
| V21 Regime特征增强 | -3.78pp(持平) | -5.86pp |

### 💡 后续方向 — 不要集中在"特征集合"或"融合规则"上

应聚焦**特征计算方式本身**的改进，而非增加/筛选特征或替换融合规则：
1. **多时间框架联动** — 周线趋势过滤日线假信号
2. **Regime编码方式创新** — 不同维度组合方式
3. **新增真正低相关的数据源** — 如海外指数、商品期货
4. **标普500ETF接入** — 海外避险资产
