...
#### 8. ⏳ 上证50/上证综指专项优化 ⭐⭐⭐
- **现状**: 上证50 BUY/SELL比0.47仍偏SELL，上证综指(-2.94%)和中证500(-2.15%)负贡献
- **方案**: 针对SIDEWAYS占比>60%的指数，调整震荡市共识阈值(如SIDEWAYS状态放宽BUY条件到3票)
...
- **方案**: 增加regime持续时长、regime转换概率、跨指数regime一致性等衍生特征
...
> 最后更新: 2026-04-11
> 
> 最新优化: V8市场状态识别 - 6状态检测+动态共识阈值+动态因子权重，超额收益+22.60%
>>>>>>> Remote
...
#### 7. ⏳ 动态止损/回撤控制 ⭐⭐⭐⭐
- **现状**: V8绝对夏普0.38偏低，最大回撤-43.84%
- **方案**: 根据regime状态动态调整止损线(如HIGH_VOL时收紧止损)，组合回撤超阈值时全局减仓
- **预期效果**: 降低最大回撤至-30%以下，提升夏普至0.55+
- **涉及文件**: `analysis/adaptive_fusion_optimizer.py`, `analysis/portfolio_backtester.py`
- **难度**: 中等
